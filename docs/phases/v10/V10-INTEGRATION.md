...
# V10 Integration — Self-Improvement + 0.1.0 Release Gate

> Read after all seven `phase-{a10..g10}-plan.md` specs. This doc covers what no single phase plan owns: shared fixtures, self-improvement harness architecture, 0.1.0 release gate verification ceremony, TE contingency activation rules, scorecard regenerator strict-mode, and the proof-run directory structure.
...
5. **Scoring phase (weekly, E10 → G10):** Recompute MEMD-10-STAR.md:
   ...
   - If any axis regresses: file recovery phase, do NOT tag 0.1.0
...
## 3. Release Gate Ceremony (G10 Phase)

**G10 is not a normal phase.** It is the release verification gate. The V10 completion gate **is** the 0.1.0 release gate.
...
- Task G10.4: Scan issue tracker for blocker+ issues tagged with any 10-STAR axis label. None allowed. If found, file recovery phase, do NOT tag 0.1.0.

**Condition 4: Reproducible proof run**
- Task G10.5: Populate `docs/verification/release-0-1-0/` with:
...
**Automatic activation (blocks 0.1.0 tag):**
...
4. If TE ≥ 4 after: proceed to 0.1.0 tag
5. If TE < 4: do NOT ship 0.1.0 until fixed
...
```markdown
## 10-Star Composite Scorecard (V10 Post-0.1.0-Release)
...
**Composite: 6.40 (0.1.0 release target) — regenerated YYYY-MM-DD by G10 harness run at strict-mode zero-generosity**

Evidence: docs/verification/release-0-1-0/YYYY-MM-DD-HARNESS-RUN.ndjson
...
4. Proof run: complete at docs/verification/release-0-1-0/
```

Append a delta-history entry so prior scorecards are reconstructible. Sign: `0.1.0-RELEASE-VERIFIED`.

## 5. Release Proof-Run Directory Structure

Harness populates `docs/verification/release-0-1-0/` with these files:
...
**Calendar spillover:** 5 graduations × 7-day clean window = 35 days post-G10. V10 code-complete and G10 harness pass close the milestone; flag-graduation runs parallel to post-release 0.1.0 maintenance window.
...
| F10 | `docs/verification/release-0-1-0/` directory | G10 release certification |
| G10 | `docs/verification/MEMD-10-STAR.md` regenerated | 0.1.0 release tag ceremony |

## 8. 0.1.0 Release Tag Ceremony
...
6. If all pass, sign `docs/verification/release-0-1-0/YYYY-MM-DD-HUMAN-REVIEW.md`.
7. Tag commit as `0.1.0` in git.
8. Publish release notes: link to proof-run directory + scorecard + delta from V9.

If any condition fails: do NOT tag. File recovery phase instead.
...
- Any axis to 9+ (0.1.0 target is 6.40; post-release roadmap is V11+)
...
- 2026-04-22 revision:
  ...
  - 0.1.0 release gate four-condition checklist added (composite, every axis ≥3, zero blockers, proof run).
