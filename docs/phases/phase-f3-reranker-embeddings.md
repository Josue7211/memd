...
B3 activates the dense signal. F3 adds the second pass that mempalace uses to reach the ceiling. Reranker is optional in mempalace — flip on with a flag, score the top-K candidates, return the reordered list. Embedding model swap is a one-line config change (`AllMiniLM-L6-v2` → `BAAI/bge-large-en-v1.5`) with empirical +3.5pp on mempalace's bench ([[.memd/lanes/architecture/A2-10-embedding-strategy.md#benchmarked-alternatives-from-longmemeval_benchpy]]).
...
- pre: LongMemEval=0.93, LoCoMo=0.55 (post-B3 baseline; if B3 not green, do not start F3)
- post: **LongMemEval ≥ 0.97**, **LoCoMo ≥ 0.55** (additional bump comes in E3)
...
- **F3-D1**: mempalace optional rerank pipeline (96.6 → 100) — [[.memd/lanes/architecture/A2-09-retrieval-pipeline.md#no-reranking-in-core-pipeline]]
- **F3-D2**: mempalace embedding bench (BGE-large +3.5pp at 1024-dim) — [[.memd/lanes/architecture/A2-10-embedding-strategy.md#benchmarked-alternatives-from-longmemeval_benchpy]]
- **F3-D3**: fastembed query-prefix convention — [[.memd/lanes/architecture/A2-10-embedding-strategy.md#memd-sidecar-embedding]]
