...
This slice verifies the current repository documents and wires one shared local memory identity/control plane across priority harness surfaces (Codex, Claude Code/OpenClaw-style hooks, and Hermes). It also records the current boundaries for federation and market claims.
...
The executable proof is `scripts/verify/feature-network-identity-federation-market-proof.sh`. It validates:
...
1. **Single user/org across app surfaces**
   ...
   - This is the corrected scope: one user/org memory identity across app surfaces, not separate identities per harness.

2. **Network identity proof linkage when present**
   - If a V26 network identity proof script exists, this proof runs it.
...
Allowed claim: memd has a local proof that priority harness surfaces are wired to one shared local memory control plane/bundle identity, and federation/market claims are explicitly bounded by existing local/synthetic or planned proof gates.
