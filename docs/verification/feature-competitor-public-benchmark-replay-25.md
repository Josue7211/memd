...
- Current proof covers **local public fixture replay** through `scripts/verify/25-5-public-benchmark-fixtures.sh` and the wrapper proof gate `scripts/verify/feature-competitor-public-benchmark-replay-proof.sh`.
...
For each fixture the proof expects both `lexical` and `memd` replay rows in `docs/verification/25-5-memory-os-runs/YYYY-MM-DD-public-benchmark-fixtures.json`.
...
- Local fixture replay: current when the proof command passes.
