...
| `feature.hive_hivemind_coordination` | `partial` | `strong` | `ad_hoc` | `none` | Strong local proof now maps archived hive coordination, roster/authority/capability contracts, executable modes, live-map guard, isolated hive-production rehearsal, and no private context broadcast; sustained production dogfood and external hive review remain unproven. |
