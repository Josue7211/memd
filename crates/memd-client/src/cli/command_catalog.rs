...
pub(crate) fn build_command_catalog(bundle_root: &Path) -> CommandCatalog {
    let commands = vec![
        ...
        entry(
            ...
            "show canonical operator state across truth, claims, freshness, and divergence",
            None,
        ),
        entry(
            "memd wake",
        ...
        entry(
            ...
            "native-cli",
            &["memd-binary", "bundle-root-present"],
            "memd checkpoint --output .memd --content \"...\" [--auto-commit] [--roadmap-set KEY=VALUE]",
            "write short-term task state into the live backend (--auto-commit commits only small tracked dirty sets, --roadmap-set patches ROADMAP_STATE)",
            None,
        ),
        entry(
            "memd claim create",
            ...
            "native-cli",
            &["memd-binary", "bundle-root-present"],
            "memd claim create --output .memd --scope task:example",
            "claim an active task scope with TTL-backed coordination state",
            None,
        ),
        entry(
            "memd claim list",
            ...
            "inspect active and expired task claims for the current bundle",
            None,
        ),
        entry(
            "memd claim close",
            ...
            "native-cli",
            &["memd-binary", "bundle-root-present"],
            "memd claim close --output .memd --scope task:example",
            "release a claim when work is done or handed off",
            None,
        ),
        entry(
            "memd remember",
        ...
        entry(
            ...
            "native-cli",
            &["memd-binary", "bundle-root-present"],
            "memd teach --output .memd --content \"fact the user just taught\"",
            "persist new user-taught facts with safe default tags/provenance",
            None,
        ),
        entry(
            "memd handoff",
        ...
        entry(
            ...
            &[
                ...
                "launcher-script-present",
                "launcher-script-executable",
                "user-taught-fact-capture",
            ],
            ".memd/agents/teach.sh --content \"fact the user just taught\"",
            "capture new user-taught facts through the teach-safe helper",
            Some(bundle_root.join("agents").join("teach.sh")),
        ),
        entry(
