...
pub(crate) async fn run_public_benchmark_command(
    ...
    if args.dry_run && args.full_eval {
        ...
        // gpt-5.4 routes through the codex-lb flat-rate proxy (no per-token
        // marginal cost); the dry-run figure below is kept as a
        // commercial-OpenAI proxy estimate for reviewers unfamiliar with the
        // flat-rate setup.
        let est_cost_full = est_tokens * 0.0000025;
        eprintln!("Dry run: {item_count} items, ~{est_calls} API calls");
        eprintln!(
            "Estimated cost: ${:.2} (gpt-5.4 OpenAI-proxy equiv); codex-lb route is flat-rate",
            est_cost_full
        );
        let mut dry_manifest = build_public_benchmark_manifest(
            args,
        ...
        if let Some(obj) = dry_manifest.runtime_settings.as_object_mut() {
            ...
            obj.insert(
                ...
                json!(args.generator_model.as_deref().unwrap_or("gpt-5.4")),
            );
        }
        return Ok(PublicBenchmarkRunReport {
            manifest: dry_manifest,
