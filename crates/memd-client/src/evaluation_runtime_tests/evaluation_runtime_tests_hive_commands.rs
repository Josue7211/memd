...
"#,
    ...
    assert!(messages[0].content.contains("next_agent_prompt:"));
    assert!(
        messages[0]
            .content
            .contains("You are taking over a memd hive handoff.")
    );
    assert!(
        messages[0]
            .content
            .contains("Before broad Git, Cargo, test, or repo-scan work")
    );
    assert!(messages[0].content.contains("host_process_scan_timeout"));
    assert!(
        messages[0]
            .content
            .contains("project_hint=host-process-scan")
    );
    assert!(messages[0].content.contains("project_hint=app-git"));
    assert!(messages[0].content.contains("native-tooling"));
    assert!(messages[0].content.contains("project_hint=host-io-report"));
    assert!(messages[0].content.contains("MEMD_HOST_IO_REPORT_TTL_SECS"));
    assert!(messages[0].content.contains("memd-continuity-status.sh"));
    assert!(
        messages[0]
            .content
            .contains("MEMD_LOOKUP_REMOTE_TIMEOUT_MS")
    );
    assert!(messages[0].content.contains("wake/live-map/host-guard"));
    assert!(messages[0].content.contains("reread_required=true"));
    assert!(messages[0].content.contains("MEMD_CODEBASE_LIVE_MAP_FRESH"));
    assert!(
        messages[0]
            .content
            .contains("MEMD_CODEBASE_LIVE_MAP_AGE_SECS")
    );
    assert!(
        messages[0]
            .content
            .contains("MEMD_CODEBASE_LIVE_MAP_TTL_SECS")
    );
    assert!(
        messages[0]
            .content
            .contains("MEMD_CODEBASE_LIVE_MAP_ACTION")
    );
    assert!(messages[0].content.contains("follow ACTION"));
    assert!(
        messages[0]
            .content
            .contains("deploy-memd-server-preflight.sh")
    );
    assert!(messages[0].content.contains("MEMD_GIT_STATUS_BLOCKERS"));
