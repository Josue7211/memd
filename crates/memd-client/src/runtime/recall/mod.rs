...
pub(crate) async fn dispatch_lookup_with_depth(
    ...
    let (result, records, tokens, hint, expansion_plan) = match depth {
        RecallDepth::Wake => {
            let res = run_wake_arm(&args, base_url).await;
            (res, 0_usize, 0_usize, None, None)
        }
        RecallDepth::Lookup => {
            ...
            match outcome {
                Ok(out) => {
                    ...
                    let plan = out.expansion_plan.clone();
                    (render_result, records, tokens, hint, Some(plan))
                }
                Err(err) => (Err(err), 0, 0, None, None),
            }
        }
        RecallDepth::Resume => {
    ...
    if !matches!(depth, RecallDepth::Wake) {
        let _ = telemetry::record(telemetry::RecordOpts {
            ...
            expansion_plan: expansion_plan.as_ref(),
        });
    }
...
pub(crate) struct LookupArmOutcome {
    ...
    pub(crate) expansion_plan: expansion::ExpansionPlan,
}
...
pub(crate) async fn run_lookup_arm_inner(
    ...
    markdown.push_str(&format!(
        ...
        expansion_plan.rationale
    ));
    if expansion_plan.ceo_mode {
        markdown.push('\n');
        markdown.push_str(&synthesis::render_ceo_packet(
            &args.query,
            &response,
            &expansion_plan,
        ));
