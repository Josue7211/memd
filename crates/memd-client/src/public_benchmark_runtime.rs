...
pub(crate) fn json_stringish_or_array_field<'a>(row: &'a JsonValue, key: &str) -> anyhow::Result<String> {
    let value = row.get(key).ok_or_else(|| anyhow!("missing {key} field"))?;
    match value {
...
pub(crate) fn public_benchmark_dataset_source(dataset: &str) -> Option<PublicBenchmarkDatasetSource> {
    match dataset {
...
pub(crate) fn resolve_public_benchmark_dataset_override_path(args: &PublicBenchmarkArgs) -> Option<PathBuf> {
...
pub(crate) fn public_benchmark_dataset_cache_metadata_path(output: &Path, benchmark_id: &str) -> PathBuf {
