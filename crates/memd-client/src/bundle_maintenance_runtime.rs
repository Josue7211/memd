...
pub(crate) fn harness_pack_enabled_for_bundle(output: &Path, agent: Option<&str>, agent_name: &str) -> bool {
    let runtime_match = read_bundle_runtime_config(output)
        .ok()
        .flatten()
        .and_then(|config| config.agent)
...
pub(crate) async fn publish_bundle_heartbeat(state: &BundleHeartbeatState) -> anyhow::Result<usize> {
    if state
        .authority_mode
        .as_deref()
...
pub(crate) fn run_capabilities_command(args: &CapabilitiesArgs) -> anyhow::Result<CapabilitiesResponse> {
...
pub(crate) fn skill_policy_apply_request(receipt: &SkillPolicyApplyArtifact) -> SkillPolicyApplyRequest {
...
pub(crate) fn write_state_artifact(path: PathBuf, content: &str, label: &str) -> anyhow::Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).with_context(|| format!("create {}", parent.display()))?;
    }
...
pub(crate) fn write_bundle_eval_artifacts(output: &Path, response: &BundleEvalResponse) -> anyhow::Result<()> {
...
pub(crate) fn read_previous_maintain_report(output: &Path) -> anyhow::Result<Option<MaintainReport>> {
    let dir = maintain_reports_dir(output);
    if !dir.exists() {
        return Ok(None);
    }
...
pub(crate) fn write_maintain_artifacts(output: &Path, response: &MaintainReport) -> anyhow::Result<()> {
