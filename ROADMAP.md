...
<!-- ROADMAP_STATE
truth_date: 2026-05-09
version: v20
version_status: evidence_ops_active_post_m0_m4
current_milestone: V20
milestone_status: evidence_ops_active_post_m0_m4
current_phase: V20-evidence-ops
phase_status: post_m0_m4_active_wip_real_evidence_pending
next_milestone: V20
...
next_step: Post-M0-M4 active WIP. Finish/verify current RAG/runtime/server and Mac bridge cleanup, scrub untracked AppleDouble/node_modules noise before commit, then continue evidence ops: clean secondary machine install, real users/devices/harness pairs, V19 auditor, V20 third-party replay. Do not tag 1.0.0 until real gates land.
...
last_handoff: docs/handoff/2026-05-09-post-m0-m4-active-wip.md
...
v5_c5_status: real_backend_locked
v5_d5_status: closed_in_process_backend
v5_e5_status: closed_in_process_backend
v5_f5_status: real_backend_locked
v6_f6_status: closed
v6_status: closed
...
v7_composite: 4.90
v8_status: closed_internal
v8_closed_internal: 2026-05-05
v8_composite: 5.10
v9_status: closed
v9_closed: 2026-05-05
v9_composite: 5.60
v10_status: closed
...
v21_v35_status: deferred_post_1_0_strategy
-->

## Status Snapshot

- truth date: `2026-05-09`
- current version: `v20` (1.0.0 ceiling substrate code complete; real dogfood and third-party replay gates pending)
- version status: `evidence_ops_active_post_m0_m4`
...
- current milestone: `V20` - 1.0.0 ceiling substrate code complete; post-M0-M4 evidence/runtime WIP is active.
...
- next step: finish/verify current RAG/runtime/server and Mac bridge WIP, scrub accidental untracked artifacts, then execute evidence ops from `docs/verification/release-1-0-0/2026-05-06-evidence-ops-start.md`; first weekly review note due 2026-05-13; do not cut `1.0.0` until elapsed-window and third-party artifacts exist.
...
| --- | --- | --- | --- |
| A6 | Episodic Ingest Pipeline | `closed` | [[docs/phases/v6/phase-a6-plan.md]] |
| B6 | Semantic Distillation | `closed` | [[docs/phases/v6/phase-b6-plan.md]] |
| C6 | Canonical Promotion | `closed` | [[docs/phases/v6/phase-c6-plan.md]] |
| D6 | Working-Context Compiler on Bench | `closed` | [[docs/phases/v6/phase-d6-plan.md]] |
| E6 | Progressive-Depth Routing | `closed` | [[docs/phases/v6/phase-e6-plan.md]] |
| F6 | Iterative Reasoning Harness | `closed` | [[docs/phases/v6/phase-f6-plan.md]] |
...
| Phase | Name | Status |
| --- | --- | --- |
| A8 | Atlas Navigation UI | `complete` |
| B8 | Correction UX | `complete` |
| C8 | Memory Inspector | `complete` |
| D8 | Provenance Browser | `complete` |
| E8 | Diff + Rollback UI | `complete` |
| F8 | Public Leaderboard Transparency Page | `complete` |
| G8 | `memd configure` Settings CLI (canonical settings surface) | `complete` |
...
V8 internal close evidence (2026-05-05): `scripts/verify/v8-operator-proof.sh` passes; proof NDJSON at `docs/verification/v8-runs/ui/operator/2026-05-05-g8-proof.ndjson` logs cost ledger visible/tunable, provenance depth 3, configure suite 7/0, and zero console errors. External stranger-review artifacts remain a public-review gate, not internal repo work.
...
| Phase | Name | Status |
| --- | --- | --- |
| A9 | Per-User Harness State Isolation | `closed` |
| B9 | Visibility/ACL Honored by Retrieval | `closed` |
| C9 | Multi-User Multi-Harness Flip | `closed` |
| D9 | Identity Collision + Adversarial Suite | `closed` |
| E9 | Correction Provenance Across Users | `closed` |
| F9 | Pre-Ship Harness Dry-Run | `closed` |
| G9 | Multi-User Adversarial Gate Harness | `closed` |

V9 gate: 2-user 3-agent dogfood holds truth across 10 sessions, divergence surfaced, no silent overwrites.

V9 close evidence (2026-05-05): `scripts/verify/v9-adversarial-suite.sh` passes; proof NDJSON at `docs/verification/v9-proof-runs/2026-05-05-adversarial-suite.ndjson` logs 8/8 scenarios passing, 8/8 negative controls firing, SC=6, CH=6, non-owned axes unchanged, composite 5.60.
...
| Phase | Name | Status |
| --- | --- | --- |
| A10 | Missed-Correction Detector + Reingest | `closed` |
| B10 | Auto-Apply Corrections Across Sessions | `closed` |
| C10 | Routine Detect/Store/Invoke/Measure/Prune | `closed` |
| D10 | Retrieval Feedback Loop | `closed` |
| E10 | Axis Integration Fixtures | `closed` |
| F10 | V10 Proof-Run Spec | `closed` |
| G10 | Scorecard Regenerator + Production-Floor Gate | `closed` |

V10 close evidence (2026-05-05): `scripts/verify/v10-self-improvement-suite.sh`
passes; proof NDJSON at
`docs/verification/v10-proof-runs/2026-05-05-self-improvement-suite.ndjson`
logs 7/7 axis scenarios passing, 4/4 negative controls firing, SC=7, CR=6,
PR=6, CH=6, RR=8, TE=5, TP=6, every axis ≥3, composite 6.40. V10 is the
production-floor milestone; `0.1.0` remains V13-owned.
...
| Phase | Name | Status |
| --- | --- | --- |
| A12 | Routine library UI (`memd routines` browse/edit/merge/deprecate) | `closed` |
| B12 | Routine composition (`memd routines compose A B --output C`) | `closed` |
| C12 | Per-project routine inheritance (`.memd/config.json` cascade) | `closed` |
| D12 | Cross-workspace export/import (`memd routines export/import`) | `closed` |
| E12 | MCP protocol shim (memd as MCP memory backend; <50 LOC client shim) | `closed` |
| F12 | ACP integration (if applicable; defer to E12 outcome) | `closed` |
| G12 | Universal-protocol parity bench + live multi-harness atomic sync | `closed` |
| H12 | Signed audit entries (ed25519) at `.memd/state/audit.ndjson` | `closed` |
| I12 | Audit UI (`memd audit browse` + `memd audit explain`) | `closed` |
| J12 | Tamper-evidence external verifier (`memd audit verify --export`) | `closed` |
...
V12 close evidence (2026-05-05): `scripts/verify/v12-interop-sota-suite.sh`
and `docs/verification/v12-proof-runs/2026-05-05-interop-sota-suite.ndjson`.
...
| Phase | Name | Status |
| --- | --- | --- |
| A13 | Public-bench domination (≥5pp margin on all four benches) | `closed` |
| B13 | Published harder-bench (current systems fail this bench) | `closed` |
| C13 | Cross-device CRDT sync (desktop ↔ mac ↔ mobile) | `closed` |
| D13 | Dormant-project recovery (30-day gap full-recall test) | `closed` |
| E13 | Behavior-inferred + multi-hop correction chains | `closed` |
| F13 | Routine auto-composition (memd suggests A+B=C) | `closed` |
| G13 | Third-party provenance replay (export + independent harness) | `closed` |
| H13 | V13 release harness (regenerates MEMD-10-STAR.md; if any axis regresses, release does not tag) | `closed` |
...
V13 close evidence (2026-05-05): `scripts/verify/v13-release-suite.sh`
and `docs/verification/release-0-1-0/2026-05-05-g13-harness.ndjson`.
...
### V21-V35: 15/20/25-Star Master Roadmap (deferred)

V21-V35 are strategy-seeded but **not active** until honest `1.0.0` close.
They do not claim new 10-STAR substrate credit; V20 already closes the memory
substrate ceiling. Stars beyond 10 measure product, network, governance, and
category maturity. Master roadmap: [[docs/strategy/25-star-master-roadmap.md]].
Evidence contract: [[docs/verification/25-star-CONTRACT.md]].

Execution rule: every version uses A-G phase commits (capability, UX, evidence
harness, dogfood, external review, kill/recovery decision, final proof packet).
No version closes on synthetic proof alone when its gate requires real users,
teams, customers, devices, orgs, external products, reviewers, or elapsed time.

| Star band | Versions | Goal | Close gate |
| --- | --- | --- | --- |
| 10-star | V20 / `1.0.0` | Perfect memory substrate | V14-V20 real evidence, V19 auditor, V20 third-party replay |
| 15-star | V21-V25 | Product/company wedge | reliability, team adoption, integrations, enterprise trust, revenue |
| 20-star | V26-V30 | Memory network | external apps/orgs/builders depend on memd with replayable trust |
| 25-star | V31-V35 | Durable memory layer | personal/org memory survives tool, team, legal, and governance shifts |

#### V21-V25: 15-Star Product Company

| Version | Outcome | Gate |
| --- | --- | --- |
| V21 | Hosted reliability | 99.9% monthly uptime, restore <15 min, zero data loss |
| V22 | Team adoption | 5 teams, 30 active days, >=70% weekly retention |
| V23 | Ecosystem integrations | 10 certified integrations, 3 external maintainers |
| V24 | Enterprise trust | SOC2-lite pack accepted by external reviewer |
| V25 | Revenue engine | 10 paying customers or signed pilots, 3 renew/expand |

#### V26-V30: 20-Star Memory Network

| Version | Outcome | Gate |
| --- | --- | --- |
| V26 | Network identity | 3 independent apps resolve same user/org memory identity without leakage |
| V27 | Federation protocol | 3 orgs exchange approved memory packets with zero private leak findings |
| V28 | Agent work market | 25 agent-delivered workflows replay from proof packets |
| V29 | Default backend push | 5 external products use memd as primary memory layer |
| V30 | Network trust | 100 certified integrations/routines, independent replay pass rate >=95% |

#### V31-V35: 25-Star Durable Memory Layer

| Version | Outcome | Gate |
| --- | --- | --- |
| V31 | Personal memory continuity | 12-month cohort migrates tools with continuity loss <=2% |
| V32 | Institutional memory | 3 orgs complete role transition without context loss or access leak |
| V33 | Legal/compliance grade | external legal/security review accepts artifacts without blocker |
| V34 | Human-agent governance | 100 contested decisions/corrections/delegations replay cleanly |
| V35 | Category completion | 25 paying orgs, 10 external products, 1 year reliability, independent review |

Recovery rule: any missed hard gate creates a `.5` recovery version scoped to
the failed gate. Recovery versions do not claim expansion credit.

Legacy V21-V25 seed remains: [[docs/strategy/v21-v25-ceo-mode.md]] and
[[docs/plans/2026-05-06-v21-v25-ceo-execution-plan.md]].
...
- **Phase 1 (complete 2026-05-02, branch `research/mining`, pending push + merge)** — Foundation wedge: `MemoryKind::Skill`, `RetrievalIntent::Skill`, atomic mirror writer, `memd skill {add,list,show,retire}`, optional `record_id` on `SkillCatalogEntry`, `## Active Skills` block in `wake.md`, in-process roundtrip test. Closing commit range: `babea58` (schema kind) → `bd98b71` (validator tighten); contract + roadmap entry at `e59b4d8`; verification ticked at `2459f42`. Final verification: F.1 workspace green except a pre-existing `working_memory_retrieval_p95_under_100ms` flake (deferred to G-phase); F.2 dogfood walked against locally-built server because homelab :8787 still ships pre-Phase-1 schema; F.3 retire honored contract §6/§10 (mirror removed, record retirement deferred to Phase 2). Plan: [[docs/superpowers/plans/2026-04-29-living-skills-phase1-foundation.md]]. Contract: [[docs/contracts/skill-record.md]].
